# NTP 時間同步模組流程 (wifi_manager — initNTP)

## 功能

ESP32 沒有 RTC 電池，開機後不知道現在幾點。
NTP (Network Time Protocol) 讓 ESP32 自動從網路伺服器取得正確時間。

## 前置條件

- `NTP_ENABLED = true`（config.h）
- `WIFI_ENABLED = true`（config.h）
- STA WiFi 連線成功（有網路存取）

## 運作原理

```
ESP32                          NTP 伺服器
  │                          (pool.ntp.org / time.nist.gov)
  │                                │
  ├─ configTime(28800, 0, ...)  ──→│  設定時區 UTC+8 + 伺服器地址
  │                                │
  │       ← NTP response ──────────┤  回傳 UTC 時間
  │                                │
  ├─ ESP32 內部 RTC 設定為           │
  │  UTC+8 的當地時間                │
  │                                │
  ▼
getLocalTime() 開始回傳正確時間
```

## 初始化流程

```
initNTP()
    │
    ├─ WiFi.status() != WL_CONNECTED ?
    │       └─ Yes: "[NTP] Skipped — no STA connection"
    │               return（跳過 NTP）
    │
    ▼ STA 已連線
印出 RTC 舊時間（debug 用，觀察漂移量）
    │
    ▼
sntp_set_sync_status(SNTP_SYNC_STATUS_RESET)  ← 重置同步狀態
    │  （深度睡眠醒來可能繼承上次的 COMPLETED 狀態，必須先清除）
    │
    ▼
configTime(NTP_GMT_OFFSET_SEC, NTP_DAYLIGHT_OFFSET_SEC, NTP_SERVER1, NTP_SERVER2)
    │
    │  NTP_GMT_OFFSET_SEC     = 28800  (UTC+8 台灣)
    │  NTP_DAYLIGHT_OFFSET_SEC = 0     (台灣無日光節約)
    │  NTP_SERVER1 = "pool.ntp.org"
    │  NTP_SERVER2 = "time.nist.gov"
    │
    ▼
等待同步（每 500ms 檢查 sntp_get_sync_status()，最多 20 次 = 10 秒）
    │
    ├─ SNTP_SYNC_STATUS_COMPLETED
    │       → _timeSynced = true
    │       → "[NTP] Time synced: 2026-03-21 14:30:00"
    │
    └─ 20 次都未 COMPLETED
            → _timeSynced = false
            → "[NTP] Sync failed — RTC time preserved as fallback"
            → RTC 舊時間保留不清零，確保無網路環境仍有近似時間
```

## 時區設定

```
┌──────────────────────────────────────────────────┐
│  configTime(GMT_offset, DST_offset, server...)   │
│                                                  │
│  GMT_offset = 28800 秒 = 8 小時 = UTC+8          │
│  DST_offset = 0 秒（台灣無夏令時間）               │
│                                                  │
│  NTP 回傳 UTC 時間 + offset = 台灣本地時間          │
│  例：UTC 06:30:00 + 8h = 14:30:00 (台灣)          │
└──────────────────────────────────────────────────┘
```

## 深度睡眠模式下的時間持久性

ESP32 的內部 RTC 運行在 RTC 電源域，即使進入深度睡眠也不會斷電：

```
開機 → NTP 同步 → RTC 設定正確時間
  │
  ▼
深度睡眠（RTC 繼續運行，使用 150kHz RC 振盪器，精度約 ±5%）
  │
  ▼
喚醒 → RTC 保留舊時間（但有漂移）
  │
  ▼
initNTP() 用 sntp_get_sync_status() 偵測是否真正校時
  │
  ├─ NTP 成功 → 修正漂移，_timeSynced = true
  └─ NTP 失敗 → 保留 RTC 舊時間作為後備，_timeSynced = false
```

### RTC 漂移量估算（150kHz RC 振盪器，±5%）

| 睡眠時長 | 最大漂移 |
|----------|---------|
| 1 小時   | ±3 分鐘 |
| 6 小時   | ±18 分鐘 |
| 12 小時  | ±36 分鐘 |
| 24 小時  | ±72 分鐘 |

### 為什麼不能用 getLocalTime() 判斷 NTP 成功

```
深度睡眠醒來
  │
  ▼
RTC 保留了上次的時間（例如 23:58，實際已經 00:00）
  │
  ▼
getLocalTime() → 回傳 true（RTC 有值 = 看起來有效）
  │               ← 這是假陽性！時間是漂移後的舊值
  │
  ▼
舊做法：誤判為「NTP 已同步」→ 用漂移時間計算排程 → 誤差累積
新做法：用 sntp_get_sync_status() → 只有真正 NTP 回應才算成功
```

## 時間同步旗標 isTimeSynced()

全域旗標，追蹤時間是否經過可靠校時：

```
設為 true 的時機：
  ├─ initNTP() 中 sntp_get_sync_status() == COMPLETED
  └─ /sync 端點收到瀏覽器時間（settimeofday 後呼叫 setTimeSynced(true)）

使用者：
  ├─ auto_logger.cpp — 只在 isTimeSynced() 時才記錄（避免用漂移時間記錄）
  └─ deep_sleep_manager.cpp — 未校時時提早 2 分鐘喚醒作為漂移補償
```

定義位置：`src/wifi_manager.cpp`（static bool _timeSynced）
宣告位置：`include/wifi_manager.h`（isTimeSynced() / setTimeSynced()）

## NTP vs 瀏覽器 /sync 比較

| 特性 | NTP 同步 | 瀏覽器 /sync |
|------|---------|-------------|
| 自動化 | 全自動 | 需有人開網頁 |
| 精準度 | ±10ms（典型） | ±1s（取決於網路延遲） |
| 網路需求 | STA WiFi + 網路 | AP 或 STA 模式皆可 |
| 深度睡眠支援 | 喚醒後自動重新同步 | 需有人開網頁 |
| 觸發時機 | 開機時（initNTP） | 網頁 onload 事件 |
| 在 config.h | `NTP_ENABLED` | 永遠可用（Web 功能） |

## 無法使用 NTP 的情境

1. **STA WiFi 連不上**（SSID/密碼錯誤、路由器不在範圍內）
2. **僅 AP 模式**（ESP32 在戶外，無路由器）
3. **路由器無網路**（區網環境，無法連到 NTP 伺服器）

以上情境：
- NTP 失敗 → `_timeSynced = false`
- RTC 舊時間保留（不清零），排程仍可運作但有漂移
- 瀏覽器 `/sync` 成功後 → `setTimeSynced(true)`
- 若兩者都失敗 → `deep_sleep_manager` 提早 2 分鐘喚醒補償漂移

## 設定參數

| 參數 | 值 | 定義位置 | 用途 |
|------|-----|---------|------|
| `NTP_ENABLED` | `true` | config.h | 功能開關 |
| `NTP_SERVER1` | `"pool.ntp.org"` | config.h | 主要 NTP 伺服器 |
| `NTP_SERVER2` | `"time.nist.gov"` | config.h | 備用 NTP 伺服器 |
| `NTP_GMT_OFFSET_SEC` | `28800` | config.h | UTC+8（台灣） |
| `NTP_DAYLIGHT_OFFSET_SEC` | `0` | config.h | 日光節約偏移（台灣=0） |
